...
class socketServer
{
        ...
        */
                ...
                foreach ($sockets as $socket) {
                    ...
                    if ($socket == $this->_master) {
                        ...
                        $bytes = @socket_recv($socket, $buffer, 2048000, 0);
    ...
    private function doEvents($socket, $recv_msg)
    {
        ...
        $msg_userid = $recv_msg['userid'];
        $msg_touserid = $recv_msg['to_userid'];
        $response = [];
        //echo "<pre>";
        switch ($msg_type) {
            case 'login':
                // 登陆上线信息
                $this->_socketPool[(int)$socket]['userInfo'] = array("userid" => $msg_userid);
                // 取得最新的名字记录
            ...
            case 'user':
                ...
                $user_list = array_column($this->_socketPool, 'userInfo');
                $response['type'] = 'user';
                $response['from'] = $userInfo['userid'];
                $response['to'] = $msg_touserid;
                $response['msg'] = $msg_content;
                $lists = $this->_socketPool;
                foreach ($lists as $item) {
                    if ($item['userInfo']['userid'] == $msg_touserid) {
                        print_r('进入了');
                        print_r($item['resource']);
                        $msg = $this->frame(json_encode($response));
                        socket_write($item['resource'], $msg, strlen($msg));
                    }
                }
                break;
        }
